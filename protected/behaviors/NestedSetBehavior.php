...
class NestedSetBehavior extends CActiveRecordBehavior
{
     ...
     * @return array
     */
    public function findPath($attribute,$array_url,$prev_url=null,$select='t.`id`,t.`rgt`,t.`lft`,t.`level`,t.`name`,t.`title`')
	{
		$owner=$this->getOwner();
		$count_path=count($array_url);
		
		$criteria=new CDbCriteria();
		$criteria->select=$select;
		$criteria->condition='t.`level`<='.($count_path+$owner->level);
		$catigories=$owner->descendants()->active()->findAll($criteria);
		
	    $level = 0;
        $level_shift=$owner->level+1;
		$breadcrumbs=array();
        $active_ids=array();
		$page=array();

	 	$url=($prev_url) ? $prev_url : '';
		
	    foreach ($catigories as $category)
	    {
	        if ($category->{$attribute}==$array_url[$level] && ($category->level-$level_shift)==$level)
			{
				$level++;

                $active_ids[$category->id]=true;
                ...
                $breadcrumbs[]=$temp; //добавляем в крошки

                $url.='/';

                $page=$temp;

				if ($level==$count_path) //последний уроень или нет
				{
					break;
                }
			}
		}
		return array('item'=>$page,'breadcrumbs'=>$breadcrumbs,'active_ids'=>$active_ids);
	}
     ...
     * @return array|string
     */
    public function findUrlForItem($attribute,$trees=false,$home_id=null,$language_id=null)
    {
        $owner=$this->getOwner();
        $url='';
        $tree=array();
        ...
        $ancestors=$owner->ancestors()->language($language_id)->findAll();

        foreach ($ancestors as $ancestor)
        {
            if ($home_id!==null && $ancestor->id==$home_id)
            {
                continue;
            }
            $url.=$ancestor->{$attribute};
            if ($trees)
            {
                $temp=$ancestor->attributes;
                $temp['url']=$url;
                $tree[]=$temp;
            }
            $url.='/';
        ...
        return array('url'=>$url,'tree'=>$tree);
    }
}