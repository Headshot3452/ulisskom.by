...
//		echo '</pre>';
    ...
    public static function getTreeForBlog($language_id)
    {
    ...
    public static function CreatePathForItem($id)
    {
        $item = Blog::model()->findByPk($id);
        $parent = self::model()->findByPk($item->parent_id);

        if($parent->level!=1)
        {
            $parents = $parent->ancestors()->findAll();
    ...
    public static function CreatePathForItemLink($id)
    {
        $item = Blog::model()->findByPk($id);
        $parent = self::model()->findByPk($item->parent_id);
        ...
        if($parent->level!=1)
        {
            ...
            foreach($parents as $key=>$value)
            {
                ...
                $result.='<a href="'.Yii::app()->createUrl('blog/index').'?category_id='.$value->id.$prev.'">'.$value->title. '</a> / ';
            }

            if(count($parents)>0)
            {
                $prev = '&prev='.array_pop($parents)->id;
            }
            else
            {
                $prev = '';
            ...
            $result.='<a href="'.Yii::app()->createUrl('blog/index').'?category_id='.$parent->id.$prev.'">'.$parent->title.'</a>';
        }
        else
        {
            $result = '<a href="'.Yii::app()->createUrl('blog/index').'?category_id='.$parent->id.'">'.$parent->title.'</a>';
        }

        return $result;
    }

    public static function CreatePathForTree($id)
    {
        $parent = BlogTree::model()->findByPk($id);
    ...
    public static function getCountNewPosts($category_id=0)
    {
        $time = time() - 24*60*60;

        if($category_id!=0)
        {
            $model = self::model()->findByPk($category_id);
            ...
            return Blog::model()->active()->count('parent_id IN ('.implode(',',$array).') AND time>:time',
                array(':time'=>$time));
        }
        else
        {
            return Blog::model()->active()->count('time>:time', array(':time' => $time));
        }
    }
}
