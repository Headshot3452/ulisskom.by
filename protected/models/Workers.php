...
class Workers extends Model
{
	...
	public function rules()
	{
		...
		return array(
			array('name, role, status', 'required'),
			array('role, status', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, role, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'orders' => array(self::HAS_MANY, 'Orders', 'worker_executor_id'),
			'orders1' => array(self::HAS_MANY, 'Orders', 'worker_staff_id'),
		);
	}
    ...
    {
        $array=array(self::ROLE_EXECUTOR=>'Исполнитель',
            self::ROLE_PICKER=>'Комплектовщик',
        );
