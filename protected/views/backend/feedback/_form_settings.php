...
if(count($data)==null):
        ?>
    <li class="form-group sortable_item" id="<?php echo $id; ?>">
        <div class="col-xs-5">
            <?php echo $form->textField($model,'name['.$id.']'); ?>
            <?php echo $form->error($model,'name'); ?>
        </div>
        <div class="col-xs-5 col-xs-offset-1">
            <?php echo $form->dropDownList($model,'type['.$id.']',SettingsFeedback::getAllType()); ?>
            <?php echo $form->error($model,'type'); ?>
        </div>
        <div class="col-xs-1">
            <img class="sort" src="/images/drag_drop.png" alt="Тяни меня" title="Тяни меня"/>
            <?php if(empty($value->system)) { ?>
                <img class="delete" id="<?php echo $id; ?>" src="/images/delete.png" alt="Удалить" title="Удалить"/>
            <?php } ?>
        </div>
    </li>
<?php endif; ?>
